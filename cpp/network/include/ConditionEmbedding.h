...
struct ConditionEmbeddingImpl : torch::nn::Module, SaveLoadMixin<ConditionEmbeddingImpl> {
    ...
    std::pair<torch::Tensor, torch::Tensor> forward(
        const std::vector<std::vector<ProtoBufCondition>>& conditions_batch);

    std::pair<torch::Tensor, torch::Tensor> forward_flattened(const nesting::FlattenInstructionsResult& flat,
                                                              int64_t batch_size);
