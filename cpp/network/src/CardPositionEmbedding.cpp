...
CardPositionEmbeddingImpl::CardPositionEmbeddingImpl(std::shared_ptr<SharedEmbeddingHolderImpl> shared_embedding_holder,
    ...
    position_mlp_hidden_ = register_module("position_mlp_hidden", torch::nn::Linear(4 * dimension_out, dimension_out));
    position_mlp_out_ = register_module("position_mlp_out", torch::nn::Linear(dimension_out, dimension_out));
    to(device, dtype);
}

torch::Tensor CardPositionEmbeddingImpl::forward(
    ...
    auto concat = torch::cat({embedded_owner, embedded_opponent_position_knowledge, embedded_top_deck_position_index,
