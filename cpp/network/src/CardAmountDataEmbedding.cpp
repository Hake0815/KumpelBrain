...
CardAmountDataEmbeddingImpl::CardAmountDataEmbeddingImpl(
  ...
  card_amount_range_embedding_ = register_module(
      "card_amount_range_embedding",
      shared_embedding_holder->card_amount_range_embedding_);
  card_position_embedding_ = register_module(
      "card_position_embedding", shared_embedding_holder->card_position_embedding_);
}

torch::Tensor CardAmountDataEmbeddingImpl::forward(
    const torch::Tensor &card_amount_data) {
  auto amount_range = card_amount_data.index({Slice(), Slice(0, 2)});
  auto position = card_amount_data.index({Slice(), 2});
  return card_amount_range_embedding_(amount_range) +
