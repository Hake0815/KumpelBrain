...
torch::Tensor
masked_self_attention_reduce(MultiHeadAttention &multi_head_attention,
  ...
  auto attention_mask = tensor_utils::make_padding_attention_mask(
      valid_token_mask, valid_token_mask.size(1),
      padded_sequences.scalar_type());
  auto attended = padded_sequences +
                  multi_head_attention(padded_sequences, padded_sequences,
                                       padded_sequences, attention_mask);
  return tensor_utils::masked_sequence_sum(attended, valid_token_mask);
}

torch::Tensor masked_attention_pooling(MultiHeadAttention &multi_head_attention,
  ...
  auto attention_mask = tensor_utils::make_padding_attention_mask(
      valid_token_mask, query.size(1), padded_sequences.scalar_type());
  return multi_head_attention(query, padded_sequences, padded_sequences,
                              attention_mask)
      .squeeze(1);
}
