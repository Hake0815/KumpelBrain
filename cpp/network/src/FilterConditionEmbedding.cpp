#include "network/include/FilterConditionEmbedding.h"
...
FilterConditionEmbeddingImpl::FilterConditionEmbeddingImpl(
    ...
    filter_field_embedding_ = register_module(
        ...
        torch::nn::Embedding(torch::nn::EmbeddingOptions(NUMBER_FILTER_FIELD_TYPES, dimension_out_).padding_idx(0)));
    filter_operation_embedding_ = register_module(
        "filter_operation_embedding",
        torch::nn::Embedding(torch::nn::EmbeddingOptions(NUMBER_FILTER_OPERATIONS, dimension_out_).padding_idx(0)));
...
torch::Tensor FilterConditionEmbeddingImpl::forward(const torch::Tensor& field_type,
                                                    const torch::Tensor& comparison_operator,
                                                    const torch::Tensor& value) {
    auto field_embedding = filter_field_embedding_(field_type.to(torch::kLong));
    auto operation_embedding = filter_operation_embedding_(comparison_operator.to(torch::kLong));

    auto value_embedding =
        torch::zeros({field_type.size(0), dimension_out_}, torch::TensorOptions().device(device_).dtype(dtype_));
    ...
    if (idx_3.numel() > 0) {
        value_embedding.index_put_({idx_3}, card_type_embedding_(value.index({idx_3}).to(torch::kLong)));
    }
    auto idx_4 = torch::nonzero(mask_4).squeeze(1);
    if (idx_4.numel() > 0) {
        value_embedding.index_put_({idx_4}, card_subtype_embedding_(value.index({idx_4}).to(torch::kLong)));
    }
    auto idx_5 = torch::nonzero(mask_5).squeeze(1);
    if (idx_5.numel() > 0) {
        value_embedding.index_put_({idx_5}, hp_embedding_(value.index({idx_5}).to(dtype_).unsqueeze(1)));
    }
