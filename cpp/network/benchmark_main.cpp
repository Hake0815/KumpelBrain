...
serialization::ProtoBufFilter make_leaf_filter(int field, int operation,
                                               int value) {
  ...
  condition->set_operation(
      static_cast<serialization::ProtoBufFilterOperation>(operation));
  condition->set_value(value);
  return filter;
}

serialization::ProtoBufFilter
make_group_filter(serialization::ProtoBufFilterLogicalOperator logical_operator,
...
serialization::ProtoBufFilter make_nested_filter(int64_t batch_index,
  ...
  const auto hp_threshold =
      static_cast<int>(40 + ((batch_index * 7 + instruction_index * 11) % 220));

  return make_group_filter(
      serialization::FILTER_LOGICAL_OPERATOR_OR,
      {
          make_group_filter(
              serialization::FILTER_LOGICAL_OPERATOR_AND,
              {
                  make_leaf_filter(serialization::FILTER_TYPE_CARD_TYPE,
                                   serialization::FILTER_OPERATION_EQUALS,
                                   card_type),
                  make_leaf_filter(
                      serialization::FILTER_TYPE_HP,
                      serialization::FILTER_OPERATION_GREATER_THAN_OR_EQUAL,
                      hp_threshold),
              }),
          make_leaf_filter(serialization::FILTER_TYPE_CARD_SUBTYPE,
                           serialization::FILTER_OPERATION_EQUALS,
                           card_subtype),
      });
}
...
serialization::ProtoBufInstructionData make_attack_data(int damage) {
  ...
  data.set_instruction_data_type(
      serialization::INSTRUCTION_DATA_TYPE_ATTACK_DATA);
...
serialization::ProtoBufInstructionData make_discard_data(int source) {
  serialization::ProtoBufInstructionData data;
  data.set_instruction_data_type(
      serialization::INSTRUCTION_DATA_TYPE_DISCARD_DATA);
  data.mutable_discard_data()->set_target_source(
      static_cast<serialization::ProtoBufTargetSource>(source));
  return data;
}
...
serialization::ProtoBufInstructionData make_card_amount_data(int min_amount,
  ...
  amount_data->set_from_position(
      static_cast<serialization::ProtoBufCardPosition>(from_position));
  return data;
}

serialization::ProtoBufInstructionData
make_return_to_deck_type_data(int return_type, int from_position) {
  serialization::ProtoBufInstructionData data;
  data.set_instruction_data_type(
      serialization::INSTRUCTION_DATA_TYPE_RETURN_TO_DECK_TYPE_DATA);
  auto *return_data = data.mutable_return_to_deck_type_data();
  return_data->set_return_to_deck_type(
      static_cast<serialization::ProtoBufReturnToDeckType>(return_type));
  return_data->set_from_position(
      static_cast<serialization::ProtoBufCardPosition>(from_position));
  return data;
}

serialization::ProtoBufInstructionData
make_filter_data(const serialization::ProtoBufFilter &filter) {
  serialization::ProtoBufInstructionData data;
  data.set_instruction_data_type(
      serialization::INSTRUCTION_DATA_TYPE_FILTER_DATA);
...
serialization::ProtoBufInstructionData make_player_target_data(int target) {
  serialization::ProtoBufInstructionData data;
  data.set_instruction_data_type(
      serialization::INSTRUCTION_DATA_TYPE_PLAYER_TARGET_DATA);
  data.mutable_player_target_data()->set_player_target(
      static_cast<serialization::ProtoBufPlayerTarget>(target));
  return data;
}
...
build_instruction_batches(int64_t batch_size, int64_t instructions_per_batch) {
...
build_condition_batches(int64_t batch_size, int64_t conditions_per_batch) {
  ...
  for (int64_t batch_index = 0; batch_index < batch_size; ++batch_index) {
    ...
    for (int64_t condition_index = 0; condition_index < conditions_per_batch;
         ++condition_index) {
...
void run_embedding_benchmarks(const torch::Device &device,
  ...
  auto shared =
      std::make_shared<SharedEmbeddingHolderImpl>(dimension, device, dtype);
  auto instruction_data_embedding =
      std::make_shared<InstructionDataEmbeddingImpl>(shared, dimension, device,
                                                     dtype);
  auto instruction_embedding = std::make_shared<InstructionEmbeddingImpl>(
      instruction_data_embedding, shared, dimension, device, dtype);
  auto condition_embedding = std::make_shared<ConditionEmbeddingImpl>(
      instruction_data_embedding, shared, dimension, device, dtype);
  ...
  std::cout << "Instruction batches: " << instruction_batch_size
            ...
            << ", flattened data rows: "
            << flat_instructions.instruction_data_type_indices.size(0) << "\n";
  std::cout << "Condition batches: " << condition_batch_size
            << ", flattened conditions: "
            << flat_conditions.instruction_indices.size(0)
            << ", flattened data rows: "
