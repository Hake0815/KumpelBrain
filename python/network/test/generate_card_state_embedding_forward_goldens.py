#!/usr/bin/env python3
"""Regenerate golden tensors for CardStateEmbedding.forward (CPU and CUDA).
...
def _forward_case(model, state_bytes: list[bytes]) -> torch.Tensor:
    with torch.inference_mode():
        return model.forward(state_bytes)
...
def generate_for_device(device: torch.device) -> dict[str, torch.Tensor]:
    ...
    for case_id in sorted(fixtures.FIXTURE_CASES.keys()):
        ...
        out = _forward_case(model, states)
        if device.type == "cuda":
...
def main() -> None:
    ...
    with _deterministic_algorithms(True):
        cpu_path = _FIXTURES_DIR / "card_state_embedding_forward_golden_cpu.pt"
        cpu_gold = generate_for_device(torch.device("cpu"))
        torch.save(cpu_gold, cpu_path)
        print(f"Wrote {cpu_path} ({len(cpu_gold)} cases)")

        if torch.cuda.is_available():
            cuda_path = _FIXTURES_DIR / "card_state_embedding_forward_golden_cuda.pt"
            ...
            print("CUDA not available; skipped card_state_embedding_forward_golden_cuda.pt")
